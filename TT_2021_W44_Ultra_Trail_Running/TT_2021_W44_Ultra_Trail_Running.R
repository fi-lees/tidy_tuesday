...
# Author: Fiona Lees
#
# Data Source: Benjamin Nowak - originally sourced from: International Trail Running Association (ITRA). 
#
# Data Description: The dataset provides a compilation of results from races that took place between 
# 2012 - 2021 and focuses on 100 mile races (give or take a few miles).
# -----------------------------------------------------------------------------------------------------
...
rankings <- readr::read_csv("https://raw.githubusercontent.com/BjnNowak/UltraTrailRunning/main/ranking.csv", col_types = list(Time = "c"))


### Wrangle Data

# Clean up ranking column names and convert time variable to period showing hours, minutes and seconds
rankings <- rankings %>% 
  janitor::clean_names() %>% 
...
# Join `race` data to `ranking` data  
race_rankings <- rankings %>% 
  left_join(race, by = "race_year_id")

# Create filtered dataset holding records for West Highland Way Race 2019  
...
whwr_race_rankings <- whwr_race_rankings %>% 
  ...
  # Runner's approx age at time of race (age variable = age in 2021, not age at time of race)
  ...
  # Rank of runner within finishing hour
  group_by(finish_hour) %>% 
  mutate(hour_rank = min_rank(time), .after = finish_hour) %>% 
  ungroup()


# Get information about the fastest / youngest / oldest runners for chart labelling.  
...
theme_update(
  ...
  plot.title = element_text(colour = "grey20", family = "Playfair Display", face = "bold", size = 18, margin = margin(t = 5, b = 5)),
...
# Chart showing number of runners who finished in each hour, by gender.  
...
# Chart showing finishing hour versus runner's age, by gender.  
...
# Tell showtext to use DPI of 300 so text size is correct in final saved chart
showtext_opts(dpi = 300)

# Save final chart as an image
...
# Load boundary data for UK
uk_sf <- ne_states(country = "united kingdom", returnclass = "sf")

# Get data for Scotland
scot_sf <- filter(uk_sf, geonunit == "Scotland")
  
# Plot route of West Highland Way Race on top of map of Scotland
p_whwr_map <- ggplot() +
  # Scotland boundary
...
# Get map image and then rescale it
map <- image_read("whwr_map.png")
map_scaled <- image_scale(map, "x500")

# Add the map to the chart and save
